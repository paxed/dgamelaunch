...
char *vi_Version = "0.0.2+dgamelaunch 1.1.2";
...
#ifdef BB_FEATURE_VI_READONLY
					&& vi_readonly == FALSE && readonly == FALSE
#endif /* BB_FEATURE_VI_READONLY */
				)
				{
					cnt = file_write (cfn, text, end - 1);
...
#ifdef BB_FEATURE_VI_COLON
static Byte *
get_one_address (Byte * p, int *addr)	// get colon addr, if present
{
	int st;
	Byte *q;
...
static Byte *
get_address (Byte * p, int *b, int *e)	// get two colon addrs, if present
{
...
colon (Byte * buf)
{
	...
	else if (strncasecmp ((char *) cmd, "delete", i) == 0)
		{														// delete lines
			if (b < 0)
				{												// no addr given- use defaults
	...
	else if (strncasecmp ((char *) cmd, "features", i) == 0)
	...
	else if (strncasecmp ((char *) cmd, "list", i) == 0)
		{														// literal print line
			if (b < 0)
				{												// no addr given- use defaults
					q = begin_line (dot);	// assume .,. for the range
	...
	else if (strncasecmp ((char *) cmd, "rewind", i) == 0)
	...
	else if (strncasecmp ((char *) cmd, "set", i) == 0)
	...
	else if (strncasecmp ((char *) cmd, "s", 1) == 0)
	...
	else if (strncasecmp ((char *) cmd, "yank", i) == 0)
		{														// yank lines
			if (b < 0)
				{												// no addr given- use defaults
					q = begin_line (dot);	// assume .,. for the range
...
static void
sync_cursor (Byte * d, int *row, int *col)
{
...
static Byte *
begin_line (Byte * p)						// return pointer to first char cur line
{
	while (p > text && p[-1] != '\n')
...
static Byte *
end_line (Byte * p)							// return pointer to NL of cur line line
{
	while (p < end - 1 && *p != '\n')
...
static Byte *
dollar_line (Byte * p)					// return pointer to just before NL line
{
	while (p < end - 1 && *p != '\n')
		p++;												// go to cur line E-o-l
	// Try to stay off of the Newline
...
static Byte *
prev_line (Byte * p)						// return pointer first char prev line
{
...
static Byte *
next_line (Byte * p)						// return pointer first char next line
{
	p = end_line (p);
	if (*p == '\n' && p < end - 1)
		p++;												// step to next line
...
static int
count_lines (Byte * start, Byte * stop)	// count line from start to stop
{
	Byte *q;
	int cnt;
...
static Byte *
find_line (int li)							// find begining of line #li
{
	Byte *q;
...
static void
dot_delete (void)								// delete the char at 'dot'
{
	(void) text_hole_delete (dot, dot);
}

static Byte *
bound_dot (Byte * p)						// make sure  text[0] <= P < "end"
{
	if (p >= end && end > text)
		{
			p = end - 1;
...
static Byte *
char_search (Byte * p, Byte * pat, int dir, int range)	// search for pattern starting at p
{
#ifndef REGEX_SEARCH
	Byte *start, *stop;
	int len;
...
static Byte *
char_insert (Byte * p, Byte c)	// insert the char c at 'p'
{
	if (c == 22)
		{														// Is this an ctrl-V?
...
static Byte *
stupid_insert (Byte * p, Byte c)	// stupidly insert the char c at 'p'
{
...
static Byte
find_range (Byte ** start, Byte ** stop, Byte c)
{
	Byte *save_dot, *p, *q;
	int cnt;
...
static int
st_test (Byte * p, int type, int dir, Byte * tested)
{
	Byte c, c0, ci;
	int test, inc;
...
static Byte *
skip_thing (Byte * p, int linecnt, int dir, int type)
{
	Byte c;
...
//  open a hole in text[]
static Byte *
text_hole_make (Byte * p, int size)	// at "p", make a 'size' byte hole
{
	Byte *src, *dest;
	int cnt;
...
static Byte *
text_hole_delete (Byte * p, Byte * q)	// delete "p" thru "q", inclusive
{
	Byte *src, *dest;
	int cnt, hole_size;
...
//
static Byte *
yank_delete (Byte * start, Byte * stop, int dist, int yf)
{
	Byte *p;
...
static void
print_literal (Byte * buf, Byte * s)	// copy s to buf, convert unprintable
{
	Byte c, b[2];
...
#if defined(BB_FEATURE_VI_YANKMARK) || defined(BB_FEATURE_VI_COLON) || defined(BB_FEATURE_VI_CRASHME)
static Byte *
string_insert (Byte * p, Byte * s)	// insert the string at 'p'
{
	int cnt, i;
...
#ifdef BB_FEATURE_VI_YANKMARK
static Byte *
text_yank (Byte * p, Byte * q, int dest)	// copy text into a register
{
	Byte *t;
	int cnt;
...
static Byte *
swap_context (Byte * p)					// goto new context for '' command make this the current context
{
	Byte *tmp;
...
static int
isblnk (Byte c)									// is the char a blank or tab
{
	return (c == ' ' || c == '\t');
}
...
static int
mysleep (int hund)							// sleep for 'h' 1/100 seconds
{
...
static Byte *
get_input_line (Byte * prompt)	// get input line- use "status line"
{
	Byte buf[BUFSIZ];
	Byte c;
	int i;
	...
	for (i = strlen ((char *) buf); i < BUFSIZ;)
		{
			...
			if (c == erase_char)
				{												// user wants to erase prev char
					...
					if (i <= 0)
						{										// user backs up before b-o-l, exit
							break;
						}
				}
			else
...
static int
file_size (Byte * fn)						// what is the byte size of "fn"
{
	struct stat st_buf;
	int cnt, sr;
...
static int
file_insert (Byte * fn, Byte * p, int size)
{
	int fd, cnt;
...
#ifdef BB_FEATURE_VI_READONLY
	if (vi_readonly == TRUE)
		goto fi1;										// do not try write-mode
#endif
...
static int
file_write (Byte * fn, Byte * first, Byte * last)
{
	int fd, cnt, charcnt;
...
static void
place_cursor (int row, int col, int opti)
{
	char cm1[BUFSIZ];
	char *cm;
	int l;
...
show_status_line (void)
{
	...
	for (cksum = l = 0; l < cnt; l++)
		{
			cksum += (int) (status_buffer[l]);
		}
...
static void
edit_status (void)							// show file status on status line
{
	int cur, tot, percent;
...
static void
format_line (Byte * dest, Byte * src, int li)
{
	int co;
	Byte c;
